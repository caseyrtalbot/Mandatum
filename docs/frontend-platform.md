...
timeline plus the session map from `FrameSnapshot` scene/theme data. Phase 3
remains underway; multi-pane layouts, remaining overlays, and broader input
parity are still explicit gaps.
...
fallback, context menu, execution timeline, and session map. Restore, multi-pane
layouts, remaining overlays, and broader input parity remain. Selecting the
